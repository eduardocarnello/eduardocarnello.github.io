...
var amendment = [
    ...
    // 2026
    { holidayDate: "20/04/2026", description: "Suspensão de expediente (Prov. CSM 2813/2025)" },
    { holidayDate: "05/06/2026", description: "Suspensão de expediente (Prov. CSM 2813/2025)" },
    { holidayDate: "10/07/2026", description: "Suspensão de expediente (Prov. CSM 2813/2025)" },
    { holidayDate: "10/07/2026", description: "Suspensão de expediente (Prov. CSM 2813/2025)" }, // Mantido conforme original (duplicado)
    { holidayDate: "07/12/2026", description: "Suspensão de expediente (Prov. CSM 2813/2025)" },
];

/* ==============================================================================================
   LISTA DE INDISPONIBILIDADES DO SISTEMA (AFETA PRAZOS FINAIS)
   ============================================================================================== */
/*
   Artigo 3º – Em primeira instância, os prazos que vencerem no dia da ocorrência de indisponibilidade 
   de quaisquer dos serviços referidos no art. 1º serão prorrogados para o dia útil seguinte à retomada 
   ...
   § 1º As indisponibilidades ocorridas entre a 0 hora e as 6 horas dos dias de expediente forense e as ocorridas 
   em feriados e finais de semana, a qualquer hora, não produzirão o efeito do caput deste artigo.
   
   § 2º Os prazos fixados em hora serão prorrogados na mesma proporção das indisponibilidades ocorridas 
   no intervalo entre 06h00 e 23h00.
...
/* ==============================================================================================
   CÁLCULO AUTOMÁTICO DA PÁSCOA (ALGORITMO DE GAUSS SIMPLIFICADO)
   ============================================================================================== */
function Easter(Y) {
...
function holidaysFunc(currentYear, expectedFinalYear) {
    ...
    for (let i = currentYear; i <= expectedFinalYear; i++) {
        nationalHolidays.push(
            // --- FERIADOS NACIONAIS E RECESSO FORENSE (Art. 116, § 2º, DO RITJSP) ---

            // Recesso Forense (Fim de Ano)
            ...
            // Fix for previous year (Correção para considerar recesso do ano anterior na contagem)
            ...
            // Fix next year (Correção para considerar recesso do próximo ano na contagem)
            ...
            // Suspensão de Prazos (Janeiro - Art. 116)
            ...
            // Fix next year (Suspensão de Prazos Janeiro próximo ano)
            ...
            // Dia do Servidor Público (com lógica de transferência para 2020 e 2025)
            {
                holidayDate: i == 2020 ? `30/10/${i}` : i == 2025 ? `27/10/${i}` : `28/10/${i}`,
                description: i == 2020 ? "Dia do Servidor Público (PROVIMENTO CSM Nº 2581/2020)" : i == 2025 ? "Dia do Servidor Público (PROVIMENTO CSM Nº 2804/2025)" : 'Dia do Servidor Público'
            },

            // Dia da Justiça (com lógica de alteração para 2022)
            {
                holidayDate: i == 2022 ? `09/12/${i}` : `08/12/${i}`,
                description: i == 2022 ? "Dia da Justiça 2022 (PROVIMENTO CSM Nº 2677/2022)" : 'Dia da Justiça'
            },

            // Consciência Negra (a partir de 2023)
            { holidayDate: i >= 2023 ? `20/11/${i}` : ``, description: i >= 2023 ? "Consciência Negra" : '' },
            { holidayDate: `25/12/${i}`, description: "Natal" },

            // --- INDISPONIBILIDADE SEVERA (SEVERE SYSTEM DOWN) ---
        ...
        cityHolidays.push(
            ...
            // Nota: Dia do Padroeiro (São Bento) movido para lógica de update abaixo
        );

        // Update - Dia do Padroeiro Marília (2023/2024/Geral)
        cityHolidays.push(
            {
